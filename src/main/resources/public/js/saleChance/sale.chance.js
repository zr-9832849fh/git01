layui.use(['table','layer'],function(){
    ...
    function delSaleChance(datas){
        ...
        layer.confirm("确定删除选中的记录",{              /* layer.confirm ：弹出框*/
            btn:['确定','取消']
        },function (index) {
            layer.close(index);          /*关闭框*/
            // ids=10&ids=20&ids=30
            var ids="ids=";
            for(var i=0;i<datas.length;i++){
                if(i<datas.length-1){          /*将最后一个多出来的 &ids= 去掉*/
            ...
            $.ajax({
                type:"post",
                url:ctx+"/sale_chance/delete",        // 删除的后台位置
                data:ids,
                dataType:"json",
    ...
    function openAddOrUpdateSaleChanceDialog(saleChanceId) {
        ...
        var url=ctx+"/sale_chance/addOrUpdateSaleChancePage";       // 这个地址是打开后台的页面（colltrnller）
        if(saleChanceId != null && saleChanceId != ""){
            title="营销机会管理-机会更新";
            url=url+"?id="+saleChanceId;    // 传给后台参数id主键
        }
        // Layui打开营销机会=添加窗口
        layui.layer.open({
            title:title,
