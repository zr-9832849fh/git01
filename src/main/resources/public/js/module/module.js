layui.use(['table', 'treetable'], function () {
    ...
    table.on('toolbar(munu-table)',function (obj) {
        switch (obj.event) {
            case "add":     // 添加目录
                ...
                break;
            case "expand":  // 全部展开
                treeTable.expandAll('#munu-table');  // 表格ID获取  expandAll：全部展开
                break;
            case "fold": // 全部折叠
                treeTable.foldAll('#munu-table'); // 表格ID获取  expandAll：全部折叠
                break;
        }
    });
    ...
    table.on('tool(munu-table)',function (obj) {
        ...
        if(layEvent === "add"){
            if(obj.data.grade==2){    // 已经是三级菜单了就不能添加了
